...
class EnchantTileAction implements ModAction, BehaviourProvider, ActionPerformer {
    ...
    EnchantTileAction(){
        ...
        actionEntry = ActionEntry.createEntry(actionId, "Enchant", "enchanting", new int[]{POLICED.getId(), NON_RELIGION.getId(),
        ...
        if (performer instanceof Player && !theTile.isEnchanted() && isCorrectEssence(activeItem, theTile)) {
            try {
                ...
                if (isActionDone(performer.getCurrentAction().getTimeLeft(), counter)) {
                    ...
                    // convert tile
                    enchantNature(performer, tileX, tileY, onSurface, tileId);
                    return true;
                }
    ...
    private void enchantNature(Creature performer, int tileX, int tileY, boolean onSurface, int tileId){
        byte tileType = Tiles.decodeType(tileId);
        Tiles.Tile theTile = Tiles.getTile(tileType);
        byte tileData = Tiles.decodeData(tileId);
        byte enchantedTileType = tileType;
        if (theTile.isGrass() && theTile.isNormal())
            enchantedTileType = Tiles.Tile.TILE_ENCHANTED_GRASS.id;
        if (theTile.isNormalTree())
            enchantedTileType = theTile.getTreeType(tileData).asEnchantedTree();
        if (theTile.isNormalBush())
            enchantedTileType = theTile.getBushType(tileData).asEnchantedBush();
